...
def main(n_seeds: int = 12) -> int:
    ...
    beat = [t["arm"] for t in table if t["beats_chance_95"] and t["arm"] not in ("A0", "AR")]
    ...
    for t in table:
        ...
        cl, ch = t["vs_chance_ci"]
        mark = "**yes**" if t["beats_chance_95"] else "no"
        if t["arm"] in ("A0", "AR"):
            mark = "—"
        L.append(f"| {t['arm']} | {t['mechanism']} | {t['damage']:,.0f} | "
                 f"{t['prevented']:,.0f} | [{lo:,.0f}, {hi:,.0f}] | {t['prevented_pct']:.1f}% | "
                 f"{t['vs_chance']:,.0f} [{cl:,.0f}, {ch:,.0f}] | {mark} | "
                 f"{t['hosp_hours_saved']:.1f} | {t['oracle_captured_pct']:.0f}% |")
    L += ["",
          ("**Arms that beat a random pick at 95%: " + (", ".join(beat) if beat else "NONE") + ".**"),
          "",
